...
function generateReport(sprints, fns) {
  ...
  const wrapped = fns.wrappedFn ? fns.wrappedFn(sprints, { tokenReport: tokens }) : null;
...
function renderWrappedSection(wrapped) {
  ...
  const highlightItems = highlights.length > 0
    ? '<ul>' + highlights.map(h => `<li><strong>[${escapeHtml(h.type)}]</strong> ${escapeHtml(h.text)}</li>`).join('') + '</ul>'
  ...
  const tokenLine = tokenSum && tokenSum.totalCostUsd > 0
  ...
  const topTagsList = (stats.topTags || []).slice(0, 5)
    .map(t => `<span class="wrapped-tag">${escapeHtml(t.tag)} (${t.count})</span>`)
    .join(' ');
