...
function trackCosts(dir) {
  ...
  if (outcomes.length === 0) {
    return {
      ...
      phaseBreakdown: {},
      costTrend: null,
      insight: 'No sprint outcome data found. Append entries to sprint_outcomes.jsonl to track costs.',
    };
  }
  ...
  for (const [sprintName, entries] of bySprint) {
    ...
    perSprint.push({
      ...
      costPerVerifiedClaim: sprintVerified > 0 ? round6(sprintCost / sprintVerified) : null,
  ...
  if (perSprint.length >= 4) {
    ...
    const firstHalf = perSprint.slice(0, mid).filter(s => s.costPerVerifiedClaim != null);
    const secondHalf = perSprint.slice(mid).filter(s => s.costPerVerifiedClaim != null);
    ...
    if (firstHalf.length > 0 && secondHalf.length > 0) {
      ...
      const changePercent = avgFirst > 0 ? Math.round((avgSecond - avgFirst) / avgFirst * 100) : null;

      costTrend = {
        direction: avgSecond < avgFirst ? 'improving' : avgSecond > avgFirst ? 'worsening' : 'stable',
  ...
  for (const k of Object.keys(modelBreakdown)) modelBreakdown[k] = round6(modelBreakdown[k]);
  for (const k of Object.keys(phaseBreakdown)) phaseBreakdown[k] = round6(phaseBreakdown[k]);

  const costPerClaim = totalClaims > 0 ? round6(totalCostUsd / totalClaims) : null;
  const costPerVerifiedClaim = totalVerifiedClaims > 0 ? round6(totalCostUsd / totalVerifiedClaims) : null;
  const sprintCount = perSprint.length;
  const avgCostPerSprint = sprintCount > 0 ? round6(totalCostUsd / sprintCount) : null;
  ...
  return {
    ...
    phaseBreakdown,
    costTrend,
    insight: generateTrackerInsight(costPerVerifiedClaim, costTrend, modelBreakdown),
  };
}
...
function generateTrackerInsight(costPerVerified, costTrend, modelBreakdown) {
  ...
  if (costPerVerified !== null) {
    if (costPerVerified < 0.01) {
      parts.push(`Excellent efficiency: $${costPerVerified.toFixed(4)} per verified claim.`);
    } else if (costPerVerified < 0.05) {
      parts.push(`Good efficiency: $${costPerVerified.toFixed(4)} per verified claim.`);
    } else {
      parts.push(`Cost per verified claim: $${costPerVerified.toFixed(4)}. Consider caching or reducing exploratory queries.`);
    }
  }

  if (costTrend) {
    if (costTrend.direction === 'improving') {
      parts.push(`Research is getting cheaper: cost per verified claim improved ${Math.abs(costTrend.changePercent)}% across recent sprints.`);
    } else if (costTrend.direction === 'worsening') {
      parts.push(`Cost per verified claim increased ${costTrend.changePercent}% in recent sprints -- investigate token usage patterns.`);
    }
  }
  ...
  if (models.length > 1) {
    ...
    parts.push(`Top model by spend: ${sorted[0]} ($${modelBreakdown[sorted[0]].toFixed(4)}).`);
  }

  return parts.length > 0 ? parts.join(' ') : 'Token cost data tracked from sprint_outcomes.jsonl.';
}
