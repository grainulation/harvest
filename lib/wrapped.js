...
const ARCHETYPES = {
  challenger: {
    ...
    emoji: 'sword',
    description: 'You stress-test everything. High challenge and contest rates mean your surviving claims are battle-hardened.',
  ...
  prototyper: {
    ...
    emoji: 'lab',
    description: 'You build to learn. Heavy prototype and tested-evidence usage means you validate through hands-on experimentation.',
  ...
  scholar: {
    ...
    label: 'The Scholar',
    emoji: 'book',
    description: 'You research deeply. High factual claim counts and documented evidence show rigorous investigation.',
  ...
  strategist: {
    ...
    emoji: 'chess',
    description: 'You drive toward decisions. High recommendation-to-factual ratios show you synthesize findings into action.',
  ...
  sentinel: {
    ...
    label: 'The Sentinel',
    emoji: 'shield',
    description: 'You spot what could go wrong. High risk identification means you protect decisions from blind spots.',
  ...
  explorer: {
    ...
    label: 'The Explorer',
    emoji: 'compass',
    description: 'You cover wide ground. Diverse topics and broad tag coverage show you leave no stone unturned.',
  ...
  balanced: {
    ...
    emoji: 'scales',
    description: 'You maintain equilibrium across research activities. A well-rounded approach with no single dominant pattern.',
...
function computeWrappedStats(sprints) {
  ...
  for (const sprint of sprints) {
    for (const c of sprint.claims || []) {
      ...
      const evidence = typeof c.evidence === 'string' ? c.evidence : (c.evidence?.tier || c.evidence_tier || 'unknown');
      evidenceDistribution[evidence] = (evidenceDistribution[evidence] || 0) + 1;
  ...
  const factualRatio = totalClaims > 0 ? (typeDistribution.factual || 0) / totalClaims : 0;
  const recommendationRatio = totalClaims > 0 ? (typeDistribution.recommendation || 0) / totalClaims : 0;
  const riskRatio = totalClaims > 0 ? (typeDistribution.risk || 0) / totalClaims : 0;
  const testedRatio = totalClaims > 0 ? (evidenceDistribution.tested || 0) / totalClaims : 0;
  const documentedRatio = totalClaims > 0 ? (evidenceDistribution.documented || 0) / totalClaims : 0;
  const challengeRatio = totalClaims > 0 ? (challengeClaims + contestedClaims) / totalClaims : 0;

  // Topic diversity: unique topics / total claims (normalized 0-1)
  const uniqueTopics = Object.keys(topicCounts).length;
  const topicDiversity = totalClaims > 0 ? Math.min(1, uniqueTopics / Math.sqrt(totalClaims)) : 0;
  ...
  return {
    ...
    topTags,
    topTopics,
    avgClaimsPerSprint: sprints.length > 0 ? Math.round(totalClaims / sprints.length) : 0,
  };
}
...
function detectPersonality(stats) {
  ...
  const priorityOrder = ['challenger', 'prototyper', 'scholar', 'strategist', 'sentinel', 'explorer', 'balanced'];
...
function computePersonalityConfidence(id, stats) {
  switch (id) {
    case 'challenger': return Math.min(1, stats.challengeRatio / 0.3);
    case 'prototyper': return Math.min(1, stats.testedRatio / 0.5);
    case 'scholar': return Math.min(1, (stats.factualRatio + stats.documentedRatio) / 0.8);
    case 'strategist': return Math.min(1, stats.recommendationRatio / 0.4);
    case 'sentinel': return Math.min(1, stats.riskRatio / 0.3);
    case 'explorer': return Math.min(1, stats.topicDiversity);
    default: return 0.5;
  }
}
...
function computeTierProgression(sprints) {
  ...
  const sprintScores = sprints.map(sprint => {
    ...
    if (claims.length === 0) return { sprint: sprint.name, avgTier: 0, claims: 0 };

    let totalWeight = 0;
    for (const c of claims) {
      const evidence = typeof c.evidence === 'string' ? c.evidence : (c.evidence?.tier || c.evidence_tier || 'stated');
...
function computeHighlights(sprints, stats) {
  ...
  // Sprint milestones
  if (sprints.length >= 10) highlights.push({ type: 'milestone', text: `${sprints.length} sprints completed` });
  if (sprints.length >= 50) highlights.push({ type: 'milestone', text: 'Power researcher: 50+ sprints' });

  // Claim milestones
  if (stats.totalClaims >= 100) highlights.push({ type: 'milestone', text: `${stats.totalClaims} claims across all sprints` });
  ...
  if (highQuality > 0 && stats.totalClaims > 0) {
    ...
    if (pct >= 30) highlights.push({ type: 'quality', text: `${pct}% of claims backed by tested or production evidence` });
  }

  // Challenge rate
  if (stats.challengeRatio > 0.1) {
    highlights.push({ type: 'rigor', text: `${Math.round(stats.challengeRatio * 100)}% challenge rate -- strong adversarial testing` });
  }

  // Top tag
  if (stats.topTags.length > 0) {
    highlights.push({ type: 'focus', text: `Most researched tag: "${stats.topTags[0].tag}" (${stats.topTags[0].count} claims)` });
  }
  ...
  if (maxSprint) {
    highlights.push({ type: 'record', text: `Most productive sprint: "${maxSprint}" with ${maxClaims} claims` });
  }
...
module.exports = { generateWrapped, computeWrappedStats, detectPersonality, ARCHETYPES };
